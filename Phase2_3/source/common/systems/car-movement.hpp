...
namespace our
{
    ...
    public:
        ...
        void update(World* world, float deltaTime, ForwardRenderer* renderer) {
            ...
            if(app->getKeyboard().isPressed(GLFW_KEY_D) && (acceleration > 0)) rotation += deltaTime * rotationSensitivity;
            if(app->getKeyboard().isPressed(GLFW_KEY_A) && (acceleration > 0)) rotation -= deltaTime * rotationSensitivity;
            if(app->getKeyboard().isPressed(GLFW_KEY_D) && (acceleration < 0)) rotation -= deltaTime * rotationSensitivity;
            if(app->getKeyboard().isPressed(GLFW_KEY_A) && (acceleration < 0)) rotation += deltaTime * rotationSensitivity;
